...
* [`src/vishon_dev/`](src/vishon_dev/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/vishon_dev/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `vishon-dev` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/vishon_dev/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns vishon-dev.example
  (:require [vishon-dev.config :as config])
